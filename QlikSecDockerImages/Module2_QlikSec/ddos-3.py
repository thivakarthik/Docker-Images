...
class Ddos(object):
    ...
    def fire(self):
        ...
        self.printHeader()
        print("TOOL STARTED..HAVE SOME PAITENCE!!".format(self.method, self.nr_workers, self.nr_sockets))

        if DEBUG:
            print("Starting concurrent workers".format(self.nr_workers))
        ...
        for i in range(int(self.nr_workers)):
            ...
            try:
                ...
                error("Failed to start".format(i))
                pass
    ...
    def stats(self):
        ...
        try:
            if self.counter[0] > 0 or self.counter[1] > 0:
                ...
                print("[+] SENDING REQUESTS!!!!. ({1} Failed)".format(self.counter[0], self.counter[1]))

                if self.counter[0] > 0 and self.counter[1] > 0 and self.last_counter[0] == self.counter[0] and self.counter[1] > self.last_counter[1]:n
...
class Striker(Process):
    ...
    def run(self):
        ...
        if DEBUG:
            print("completed run. Sleeping...".format(self.name))
...
def usage():
    ...
    print()
    print(' USAGE: ./ddos.py <url> [OPTIONS]')
    print()
    print(' OPTIONS:')
